...
package net.sf.rudetools.plugin.now.shell.actions;

import java.text.MessageFormat;
import java.util.StringTokenizer;

import net.sf.rudetools.plugin.now.shell.Resource;
import net.sf.rudetools.plugin.now.shell.ResourceUtils;
...
public class EasyShellAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		if (!isEnabled()) {
			MessageDialog.openInformation(new Shell(), "Easy Shell",
					"Wrong Selection");
			ShellNowPlugin.log("Wrong Selection");
			return;
		}

		String ActionIDStr = action.getId();
		ShellNowPlugin.getDefault().sysout(true,
				"Action ID: >" + ActionIDStr + "<");
		String[] EasyShellActionStr = {
		...
		if (ActionIDNum == -1) {
			MessageDialog.openInformation(new Shell(), "Easy Shell",
					"Wrong Action ID");
			ShellNowPlugin.log("Wrong Action ID");
			return;
		}
		...
		for (int i = 0; i < resource.length; i++) {
			...
			if (full_path != null) {
				...
				try {
					String target = ShellNowPlugin.getDefault().getTarget(
							ActionIDNum);
					EasyShellQuotes quotes = ShellNowPlugin.getDefault()
							.getQuotes();
					...
					// args[4] = autoQuotes(resource[i].getProjectName(),
					// EasyShellQuotes.quotesDouble);
					...
					if (ActionIDNum == 3) {
						String cmd = fixQuotes(
								MessageFormat.format(target, (Object[]) args),
								quotes);
						ShellNowPlugin.getDefault().sysout(true, "--- clp: >");
						cmdAll += cmd;
						ShellNowPlugin.getDefault().sysout(true, cmd);
						ShellNowPlugin.getDefault().sysout(true, "--- clp: <");
					}
					// handling command line
					else {
						// string tokenizer enabled ?
						if (ShellNowPlugin.getDefault().isTokenizer()) {
							...
							while (st.hasMoreElements()) {
								cmds[c] = fixQuotes(MessageFormat.format(
										st.nextToken(), (Object[]) args),
										quotes);
								ShellNowPlugin.getDefault().sysout(true,
										cmds[c]);
								c++;
						...
						// the old command line passing without string tokenizer
							...
							String cmd = fixQuotes(MessageFormat.format(target,
									(Object[]) args), quotes);
							ShellNowPlugin.getDefault().sysout(true,
									"--- cmd: >");
				...
				ShellNowPlugin.log("Unable to open shell");
				return;
	...
	private String autoQuotes(String str, EasyShellQuotes quotes) {
		...
		if (quotes == EasyShellQuotes.quotesSingle) {
			...
			ret = "\"" + str + "\"";
		} else if (quotes == EasyShellQuotes.quotesAuto
				&& (str.indexOf("\"") == -1) && (str.indexOf(" ") != -1)) { // if
																			// no
																			// quotes
																			// and
																			// space
	...
	private String fixQuotes(String str, EasyShellQuotes quotes) {
		String ret = str;
		/*
		 * if ( (quotes == EasyShellQuotes.quotesYes) || (quotes ==
		 * EasyShellQuotes.quotesAuto) ) { // try to replace known combinations
		 * //ret = Matcher.quoteReplacement(str).replaceAll("./\"", "\"./"); ret
		 * = str.replaceAll("./\"", "\"./"); }
		 */
		return ret;
	}
