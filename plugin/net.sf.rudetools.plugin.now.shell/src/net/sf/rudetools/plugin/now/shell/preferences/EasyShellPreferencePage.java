...
package net.sf.rudetools.plugin.now.shell.preferences;
...
public class EasyShellPreferencePage extends PreferencePage implements
	...
	private enum EasyShellCommand {
		cmdUnknown(0, "Unknown shell / file browser", "open {1}",
				...
				"explorer.exe /select, {2} ", "{2}{5}"), cmdWinConEmu(12,
		...
		// construct
		EasyShellCommand(int id, String label, String openCmd, String runCmd,
				String exploreCmd, String copyPathCmd) {
			this.id = id;
			this.label = label;
	...
	public enum EasyShellQuotes {
		quotesNo(0, "No"), quotesSingle(1, "Single"), quotesDouble(2, "Double"), quotesAuto(
				3, "Automatic"); // check if no quotes and space in string, then
									// add
		// attributes
		private final int id;
	...
	public enum EasyShellTokenizer {
		EasyShellTokenizerNo(0, "No"), EasyShellTokenizerYes(1, "Yes");
		// attributes
		private final int id;
		private final String mode;
	...
	private enum LinuxDesktop {
		desktopUnknown(0, "Unknown"), desktopCde(1, "CDE"), desktopKde(2, "KDE"), desktopGnome(
				3, "Gnome");
		// desktopXfce(4, "Xfce");
		// attributes
		private final int id;
	...
	public EasyShellPreferencePage() {
		setPreferenceStore(ShellNowPlugin.getDefault().getPreferenceStore());
	...
	private void initializeDefaults() {
		...
		store.setDefault(P_TOKENIZER_LIST_STR,
				EasyShellTokenizer.EasyShellTokenizerYes.name());
	}
	...
	protected Control createContents(Composite parent) {
		...
		// ---------------------------------- Open
		// ---------------------------------------------------
		targetOpenEditor = new StringFieldEditor(P_TARGET,
				"&Shell Open command:", targetColumn);
		targetOpenEditor.getLabelControl(targetColumn).setToolTipText(
				targetTooltip);

		// ----------------------------------- Run
		// ---------------------------------------------------
		targetRunEditor = new StringFieldEditor(P_TARGET_RUN,
				"&Shell Run command:", targetColumn);
		targetRunEditor.getLabelControl(targetColumn).setToolTipText(
				targetTooltip);

		// --------------------------------- Explore
		// -------------------------------------------------
		targetExploreEditor = new StringFieldEditor(P_TARGET_EXPLORE,
				"&Explore command:", targetColumn);
		targetExploreEditor.getLabelControl(targetColumn).setToolTipText(
				targetTooltip);

		// --------------------------------- Copypath
		// ------------------------------------------------
		targetCopyPathEditor = new StringFieldEditor(P_TARGET_COPYPATH,
				"Copy Path string:", targetColumn);
		targetCopyPathEditor.getLabelControl(targetColumn).setToolTipText(
				targetTooltip);
	...
	private boolean loadStore() {
		IPreferenceStore store = getPreferenceStore();
		targetCombo.select(store.getInt(P_LIST) + 1); // get index: just for
														// compatibility to old
														// EasyShell version <=
														// 1.2.0
		String IdStr = store.getString(P_LIST_STR); // get name: new method >=
													// 1.3.0
		if (IdStr != null && IdStr.length() > 0) {
			targetCombo.select(EasyShellCommand.valueOf(IdStr).getId());
		}
		ShellNowPlugin.getDefault().sysout(true,
				"Open Value      : >" + store.getString(P_TARGET) + "<");
		ShellNowPlugin.getDefault().sysout(true,
				"Open Default    : >" + store.getDefaultString(P_TARGET) + "<");
		targetOpenEditor.setStringValue(store.getString(P_TARGET));
		ShellNowPlugin.getDefault().sysout(true,
				"Run Value       : >" + store.getString(P_TARGET_RUN) + "<");
		ShellNowPlugin.getDefault().sysout(
				true,
				"Run Default     : >" + store.getDefaultString(P_TARGET_RUN)
						+ "<");
		targetRunEditor.setStringValue(store.getString(P_TARGET_RUN));
		ShellNowPlugin.getDefault()
				.sysout(true,
						"Explore Value   : >"
								+ store.getString(P_TARGET_EXPLORE) + "<");
		ShellNowPlugin.getDefault().sysout(
				true,
				"Explore Default : >"
						+ store.getDefaultString(P_TARGET_EXPLORE) + "<");
		targetExploreEditor.setStringValue(store.getString(P_TARGET_EXPLORE));
		ShellNowPlugin.getDefault().sysout(
				true,
				"Copypath Value  : >" + store.getString(P_TARGET_COPYPATH)
						+ "<");
		ShellNowPlugin.getDefault().sysout(
				true,
				"Copypath Default: >"
						+ store.getDefaultString(P_TARGET_COPYPATH) + "<");
		targetCopyPathEditor.setStringValue(store.getString(P_TARGET_COPYPATH));
		quotesCombo.select(EasyShellQuotes.quotesNo.getId()); // just for the
																// case it's new
		...
		debugCombo.select(EasyShellDebug.debugNo.getId()); // just for the case
															// it's new
		...
		TokenizerCombo.select(EasyShellTokenizer.EasyShellTokenizerYes.getId()); // just
																					// for
																					// the
																					// case
																					// it's
		...
		if (TokenizerStr != null && TokenizerStr.length() > 0) {
			TokenizerCombo.select(EasyShellTokenizer.valueOf(TokenizerStr)
					.getId());
		}
		return true;
	}
	...
	private boolean saveStore() {
		...
		store.setValue(P_LIST, targetCombo.getSelectionIndex() - 1); // just for
																		// compatibility
																		// to
																		// old
																		// EasyShell
																		...
																		// <=
																		// 1.2.0
		store.setValue(P_LIST_STR,
				EasyShellCommand.getFromId(targetCombo.getSelectionIndex())
						.name());
		store.setValue(P_QUOTES_LIST_STR,
				EasyShellQuotes.getFromId(quotesCombo.getSelectionIndex())
						.name());
		store.setValue(P_DEBUG_LIST_STR,
				EasyShellDebug.getFromId(debugCombo.getSelectionIndex()).name());
		store.setValue(P_TOKENIZER_LIST_STR,
				EasyShellTokenizer
						.getFromId(TokenizerCombo.getSelectionIndex()).name());
		return true;
	}
	...
	private void refreshTarget() {
		...
		/*
		 * we do not want set preferences now, will be set with apply or OK
		 * (performOk) saveStore();
		 */
	}
	...
	private void setToDefaults() {
		...
		store.setToDefault(P_LIST); // just for compatibility to old EasyShell
									// version <= 1.2.0
	...
	private EasyShellCommand getProperCommand() {
		...
		/*
		 * possible OS string: AIX Digital UNIX FreeBSD HP UX Irix Linux Mac OS
		 * Mac OS X MPE/iX Netware 4.11 OS/2 Solaris Windows 95 Windows 98
		 * Windows NT Windows Me Windows 2000 Windows XP Windows 2003 Windows CE
		...
		} else if (osname.indexOf("unix") != -1 || osname.indexOf("irix") != -1
	...
	private LinuxDesktop detectDesktop() {
		...
		try {
			...
			while ((line = in.readLine()) != null
					&& resultCode == LinuxDesktop.desktopUnknown) {
			...
			if (result != 0) {
				...
				// stdout for debugging purposes
				ShellNowPlugin.getDefault().sysout(true,
						"detectDesktop return code: " + result);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 *
	 * @see http 
	 *      ://techbase.kde.org/KDE_System_Administration/Environment_Variables
	 *      #KDE_FULL_SESSION
	 */
	...
	/**
	 * TODO: detects Xfce desktop private boolean isXfce() { return false; }
	 */
	 ...
	 * @see detectDesktop
	 */
	private boolean isExpectedCommandOutput(String[] command,
			String expectedOutput, boolean toLowerCase) {
		boolean found = false;
		try {
