...
public class FileTypeInf {
	...
	public synchronized void addDone(String fileType, Long newDone) {
		...
		if (index >= 0) {
			...
			LOG.error("XXXXXXXXXXXXXXX new done type:\t{} with done:{}",
					fileType, newDone);
		}
		sumDone += newDone;
	}
	...
	public synchronized String getText(String fileType) {
		int index = typeList.indexOf(fileType);
		if (index >= 0) {
			return format(numDone.get(index)) + " / "
					+ format(numAll.get(index));
		}
		return " - ";
	}
	...
	public synchronized String getText(int columnIndex) {
		...
		if (columnIndex == 0) {
			...
			text = format(numDone.get(columnIndex - 1)) + " / "
					+ format(numAll.get(columnIndex - 1));
		}
		return text;
	}
	...
	public int getPercent() {
		if (sumAll > 0) {
			String strAll = String.valueOf(sumAll);
			String strDone = String.valueOf(sumDone);
			int cutLength = strAll.length() - 3;
			if (strDone.length() > cutLength) {
				String str3ForAll = strAll.substring(0, strAll.length()
						- cutLength);
				String str3ForDone = strDone.substring(0, strDone.length()
						- cutLength);
				int intAll = Integer.parseInt(str3ForAll);
				int intDone = Integer.parseInt(str3ForDone);
				double percent = intDone * 100.0 / intAll;
				return (int) percent;
			} else {
